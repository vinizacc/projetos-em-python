...
def build_2d():
    ...
    clear_variables()
    ax.clear()  # Limpa o conteúdo do gráfico, incluindo textos e números
    ax.set_xlim(0, 1000)  # Define os limites do eixo X
    ax.set_ylim(0, 1000)  # Define os limites do eixo Y
...
def toggle_3d_2d(event):
    """Alterna entre os modos 2D e 3D."""
    global is_3d
    is_3d = not is_3d
    if is_3d:
        ax.clear()
...
def add_point_2d(event):
    try:
        ...
        # ax.remove()  # Limpa apenas o conteúdo do gráfico
        ax = fig.add_subplot(111)  # Recria o gráfico como 2D
...
fig = plt.figure(figsize=(15, 12), facecolor='grey')  # Tamanho da figura
